AutoNet - Network Automation Toolchain
==============================

This code in this repository is used for the following tasks:

    - use the database at https://github.com/example-org/peering to generate all
      IXP peering
...
AutoNet IS AUTHORITATIVE:
-----------------------

    The 'autonet' repository is the authoritative source for BIRD configurations in
    the network. Any changes to the BIRD configuration on the routers
...
------------------
	...
	├── update-routers.sh
	└── vars
	    ├── router1.example.net.yml
	    ├── router2.example.net.yml
	    ├── router3.example.net.yml
	    ├── generic.yml                    <- generic AutoNet settings and inventory for routers and IXPs
	    ├── members_bgp.yml                <- ips & prefixes of members with BGP
	    ├── statics-dc5.yml                <- static routes location1
	    ├── statics-eunetworks.yml         <- static routes location2
...
BIRD 2 has IPv4 and IPv6 support in the same process, although AutoNet isn't built for this.
...
Note that AutoNet currently requires the ability to write to the
...
To prepare a Debian machine to build AutoNet:
...
Copyright (c) 2017-2023, Network Operators <routers@example.net>

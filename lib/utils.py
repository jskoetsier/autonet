...
def get_config_value(
    key: str, default: Any = None, config_file: str = "vars/generic.yml"
) -> Any:
    """
...
def create_backup_file(
    source_path: Union[str, Path], backup_suffix: str = None
) -> Optional[Path]:
    """
...
def cleanup_old_files(
    directory: Union[str, Path], pattern: str, max_age_days: int
) -> int:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config-file", default="vars/generic.yml", help="Configuration file"
    )
    parser.add_argument("--default", help="Default value for configuration")
    parser.add_argument("--validate-dir", help="Validate directory")
    parser.add_argument(
        "--create-dir", action="store_true", help="Create directory if missing"
    )
    ...
    try:
        if args.get_config:
            ...
            if validate_directory(
                args.validate_dir, create=args.create_dir, writable=True
            ):

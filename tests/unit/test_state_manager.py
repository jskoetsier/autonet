...
from lib.state_manager import StateManager, StateEvent, GenerationRecord, DeploymentRecord, EventType
...
class TestStateManager(unittest.TestCase):
    ...
    def test_performance_stats(self):
        ...
        # Add some test data
        self.manager.track_generation(GenerationRecord(
            config_hash="test1",
            peer_count=3,
            duration_ms=1500,
            memory_peak_mb=30.0,
    ...
    def test_cleanup_old_data(self):
        ...
        old_event = StateEvent(
            ...
            component="test",
            message="Old event",
            timestamp=datetime.now() - timedelta(days=40)  # Older than default retention
        )
    ...
    def test_event_filtering_by_type(self):
        ...
        self.manager.track_event(StateEvent(
            event_type=EventType.GENERATION_SUCCESS,
            component="test",
        ...
        success_events = self.manager.get_recent_events(10, EventType.GENERATION_SUCCESS)
        self.assertEqual(len(success_events), 1)
        self.assertEqual(success_events[0].message, "Success event")
        
        # Get only failure events
        failure_events = self.manager.get_recent_events(10, EventType.GENERATION_FAILURE)
